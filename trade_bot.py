...
class TicketSetupModal(discord.ui.Modal, title="仲介チケット作成 - 詳細入力"):
    ...
    target_name = discord.ui.TextInput(
        label="取引相手のユーザー名", 
        placeholder="例: user_name (IDではなく名前を入力)", 
        min_length=2, 
        max_length=32, 
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # --- 名前からユーザーを検索するロジックに変更 ---
        target_user = guild.get_member_named(self.target_name.value)
        
        if target_user is None:
            return await interaction.response.send_message(
                f"❌ 「{self.target_name.value}」というユーザーが見つかりませんでした。\n"
                "※名前が正しいか、または相手がサーバーに参加しているか確認してください。", 
                ephemeral=True
            )
        ...
        channel = await guild.create_text_channel(
            ...
            topic=f"依頼者:{user.name} | 相手:{target_user.name} | 内容:{self.trade_item.value}"
        )
...
# --- 以下、TicketLaunchView と MyBot クラスは前回の配置のまま使用 ---
