...
@bot.tree.command(name="profile", description="スタッフ実績")
async def profile(interaction: discord.Interaction, user: discord.Member):
    ...
    async for msg in log_ch.history(limit=500):
        if not msg.embeds: continue
        emb = msg.embeds[0]
        if "取引完了記録" in emb.title and str(user.id) in emb.footer.text: total += 1
        elif "レビュー" in emb.title and str(user.id) in emb.fields[2].value if len(emb.fields)>2 else "":
            stars.append(len(emb.fields[0].value))
    
    avg = sum(stars)/len(stars) if stars else 0
    embed = discord.Embed(title=f"👤 {user.display_name}", color=discord.Color.green())
    embed.add_field(name="累計仲介", value=f"{total}回", inline=True)
    embed.add_field(name="平均評価", value=f"★{avg:.1f}", inline=True)
