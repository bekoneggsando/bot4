...
class TicketSetupModal(discord.ui.Modal, title="仲介チケット作成 - 詳細入力"):
    target_id = discord.ui.TextInput(label="取引相手のユーザーID", placeholder="18桁の数字を入力", min_length=17, max_length=20, required=True)
    trade_item = discord.ui.TextInput(label="取引内容", placeholder="例: アカウント、アイテムなど", required=True)
    price = discord.ui.TextInput(label="取引価格", placeholder="例: 5,000円", required=True)
    payment = discord.ui.TextInput(label="支払い方法", placeholder="例: PayPay、銀行振込", required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.response.send_message("❌ 取引相手が見つかりません。IDが正しいか確認してください。", ephemeral=True)

        # チャンネルの閲覧権限設定（本人・相手・スタッフ・Botのみ）
        overwrites = {
        ...
        channel = await guild.create_text_channel(
            ...
            topic=f"依頼者:{user.id} | 相手:{target_user.id} | 内容:{self.trade_item.value}"
        )
        ...
        embed.set_footer(text="スタッフが参ります。完了後は /finish を実行してください。")
...
class TicketLaunchView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None) # ボタンを永続化
