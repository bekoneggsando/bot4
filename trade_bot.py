...
REPORT_CATEGORY_ID = 1484056755343458355  # 通報用
BUG_CATEGORY_ID = 1484056835647340614    # 不具合報告用
...
class ReportView(discord.ui.View):
    ...
    @discord.ui.button(label="通報・違反報告を行う 🚨", style=discord.ButtonStyle.danger, custom_id="persistent_report")
...
class BugView(discord.ui.View):
    ...
    @discord.ui.button(label="不具合・バグを報告する 🛠️", style=discord.ButtonStyle.secondary, custom_id="persistent_bug")
...
class ReportModal(discord.ui.Modal, title="ユーザー通報・違反報告"):
    target_user = discord.ui.TextInput(label="通報対象（名前やID）", placeholder="例: user_name#0000", required=True)
    reason = discord.ui.TextInput(label="通報内容", style=discord.TextStyle.paragraph, placeholder="違反内容を詳しく記入してください。", required=True)
    evidence = discord.ui.TextInput(label="証拠", placeholder="メッセージリンクや画像URLなど", required=False)
...
class BugModal(discord.ui.Modal, title="不具合・バグ報告"):
    ...
    description = discord.ui.TextInput(label="内容", style=discord.TextStyle.paragraph, placeholder="起きた現象を詳しく教えてください。", required=True)
...
async def create_custom_ticket(interaction: discord.Interaction, prefix, category_id, content, color):
    ...
    embed = discord.Embed(title=f"{prefix}チケット", color=color, description=content)
    await ticket_channel.send(content=f"{interaction.user.mention} 運営が内容を確認します。証拠画像等があればここに貼ってください。", embed=embed)
...
async def setup_support(interaction: discord.Interaction):
    ...
    report_embed = discord.Embed(
        title="🚨 通報・違反報告窓口",
        description="詐欺の疑い、迷惑行為、規約違反を発見した場合はこちら。\n※内容は運営のみに共有されます。",
        color=discord.Color.red()
    )
    await interaction.channel.send(embed=report_embed, view=ReportView())

    # 不具合用パネル
    bug_embed = discord.Embed(
        title="🛠️ 不具合・バグ報告窓口",
        description="Botが動かない、エラーが出る、表示がおかしい場合はこちらから報告してください。",
        color=discord.Color.orange()
    )
    await interaction.channel.send(embed=bug_embed, view=BugView())
    
    await interaction.response.send_message("2つのパネルを設置しました！", ephemeral=True)
